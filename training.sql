...
SELECT
    ...
    CASE
        WHEN m.element = 'Fire' THEN 'Fire'
        WHEN m.element = 'Water' THEN 'Water'
        WHEN m.element = 'Dragon' THEN 'Dragon'
        WHEN m.element = 'Grass' THEN 'Grass'
        ELSE 'Normal'
    END AS element
FROM
    pokemon p
JOIN
    multipliers m ON p.element_id = m.id
WHERE
